...
export const SCENES: Scene[] = [
  ...
  {
    slug: "hero-map",
    title: "Hero · המסלול מצויר על האור",
    note: "**לא אריחי מפה.** הגרסה הראשונה השתמשה ב-OpenStreetMap אמיתי ודווחה כשגויה — ״זו התמונה מהמפה, ומה שהצעת היה ציור״. עכשיו זו SVG שמוקרנת בשרת: אור הטיול נשאר, והמסלול הוא קו מעליו. אפס Leaflet, אפס אריחים, אפס בקשות רשת. קווי הרשת הם קווי אורך ורוחב אמיתיים בקנה המידה של הציור — לא חופים מומצאים",
    bleed: true,
    render: () => (
      <AuraHero
        tripId={f.TRIP_ID}
  ...
  },
  {
    ...
    title: "Hero · טיול בלי קואורדינטות",
    note: "אותו Hero בלי המסלול. זה כבר לא ״פולבק״ אלא פשוט שכבה אחת פחות — האור תמיד שם, והציור מצטרף אליו כשיש מה לצייר",
    bleed: true,
    render: () => (
      <AuraHero
        tripId={f.TRIP_ID}
